...
class SQL{
    ...
    public static function update($column,$table,$whereequals,$to){
        ...
        if(is_array($column)){
            ...
            $squery .= self::getInstance()->where($whereequals);
            $query = $sql->prepare($squery);
        }else{
            $values[":value"] = $to;
            $query = $sql->prepare("UPDATE ".$table." SET ".$column."=:value".self::getInstance()->where($whereequals));
        }
    ...
    public static function delete($table,$whereequals){
        ...
        $query = self::getInstance()->connection->prepare("DELETE FROM ".$table."".self::getInstance()->where($whereequals));
        self::getInstance()->bind($query,$whereequals)->execute();
    }
